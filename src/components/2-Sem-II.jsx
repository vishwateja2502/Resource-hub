...
function FilePopup({ open, onClose, fetchUrl, title, isDark }) {
  const [files, setFiles] = React.useState([]);
  const [error, setError] = React.useState('');
  const [pdfModalOpen, setPdfModalOpen] = React.useState(false);
  const [pdfToView, setPdfToView] = React.useState('');
    ...
    <>
      <div style={{position: 'fixed', top:0, left:0, width:'100vw', height:'100vh', background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center'}}>
        <div style={{background:isDark?'#232a39':'#fff', color:isDark?'#fff':'#222', borderRadius:12, padding:24, width:'90vw', maxWidth:800, maxHeight:'90vh', overflow:'auto', position:'relative'}}>
          ...
          <h1 style={{color:isDark?'#fff':'#333', fontSize:28, marginBottom:16}}>{title}</h1>
...
export default function TwoSemII() {
  ...
  const [showDBMS, setShowDBMS] = React.useState(false);
  const [showOOP, setShowOOP] = React.useState(false);
    ...
    <div className={`min-h-screen w-full ${isDark ? 'bg-[#181e29]' : 'bg-gray-100'}`} style={{position:'relative',overflow:'hidden'}}>
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className={`${isDark ? 'bg-[#232a39] border-[#232a39]' : 'bg-white border-gray-200'} border p-8 rounded-2xl w-full max-w-2xl shadow-xl relative`}>
          ...
          <h2 className={`${isDark ? 'text-white' : 'text-black'} text-2xl font-bold mb-8`}>2nd Year - Semester II Subjects</h2>
          <div className="flex flex-col gap-4">
            <button className="py-3 bg-red-300 text-black rounded-lg font-semibold hover:bg-red-400 transition duration-300" onClick={() => setShowCO(true)}>Computer-Organisation</button>
            <button className="py-3 bg-red-300 text-black rounded-lg font-semibold hover:bg-red-400 transition duration-300" onClick={() => setShowFLAT(true)}>FLAT</button>
            <button className="py-3 bg-red-300 text-black rounded-lg font-semibold hover:bg-red-400 transition duration-300" onClick={() => setShowSE(true)}>Software-Engineering</button>
            <button className="py-3 bg-red-300 text-black rounded-lg font-semibold hover:bg-red-400 transition duration-300" onClick={() => setShowDBMS(true)}>DBMS</button>
            <button className="py-3 bg-red-300 text-black rounded-lg font-semibold hover:bg-red-400 transition duration-300" onClick={() => setShowOOP(true)}>OOP</button>
          </div>
        </div>
      </div>
      <FilePopup open={showCO} onClose={() => setShowCO(false)} fetchUrl="http://localhost:5000/list-co-files" title="📄 Computer-Organisation Resources" isDark={isDark} />
      <FilePopup open={showFLAT} onClose={() => setShowFLAT(false)} fetchUrl="http://localhost:5000/list-flat-files" title="📄 FLAT Resources" isDark={isDark} />
      <FilePopup open={showSE} onClose={() => setShowSE(false)} fetchUrl="http://localhost:5000/list-se-files" title="📄 Software-Engineering Resources" isDark={isDark} />
      <FilePopup open={showDBMS} onClose={() => setShowDBMS(false)} fetchUrl="http://localhost:5000/list-dbms-files" title="📄 DBMS Resources" isDark={isDark} />
      <FilePopup open={showOOP} onClose={() => setShowOOP(false)} fetchUrl="http://localhost:5000/list-oop-files" title="📄 OOP Resources" isDark={isDark} />
      {showSettings && (
