...
function EnglishPdfPopup({ open, onClose, isDark }) {
  ...
  useEffect(() => {
    if (!open) return;
    fetch('http://localhost:5000/list-english-pdfs')
      .then(res => res.json())
      .then(data => {
        setFiles(data);
        setError('');
  ...
  return (
    <>
      <div style={{position: 'fixed', top:0, left:0, width:'100vw', height:'100vh', background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center'}}>
        <div style={{background:isDark?'#232a39':'#fff', borderRadius:12, padding:24, width:'90vw', maxWidth:800, maxHeight:'90vh', overflow:'auto', position:'relative'}}>
          <button onClick={onClose} style={{position:'absolute', top:12, right:16, fontSize:24, background:'none', border:'none', cursor:'pointer', color: isDark ? '#fff' : '#222'}}>&times;</button>
          <h1 style={{color:isDark?'#fff':'#333', fontSize:28, marginBottom:16}}>📄 English Resources</h1>
                ...
                <div key={url} style={{display: 'inline-flex', alignItems: 'center', margin: 0, gap: 8}}>
                  <button
                    ...
                    style={{
                      margin: 0,
                      padding: '10px 28px',
                      background: isPdf ? '#fca5a5' : '#a5d8fc',
                      color: '#222',
                      border: 'none',
                      borderRadius: 8,
                      fontWeight: 600,
...
function PPS2FilesPopup({ open, onClose, isDark }) {
  const [files, setFiles] = useState([]);
  const [error, setError] = useState('');
  const [pdfModalOpen, setPdfModalOpen] = useState(false);
  const [pdfToView, setPdfToView] = useState('');
  ...
  useEffect(() => {
    if (!open) return;
    fetch('http://localhost:5000/list-pps2-files')
      .then(res => res.json())
      .then(data => {
        setFiles(data);
        setError('');
  ...
  return (
    <>
      <div style={{position: 'fixed', top:0, left:0, width:'100vw', height:'100vh', background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center'}}>
        <div style={{background:isDark?'#232a39':'#fff', borderRadius:12, padding:24, width:'90vw', maxWidth:800, maxHeight:'90vh', overflow:'auto', position:'relative'}}>
          <button onClick={onClose} style={{position:'absolute', top:12, right:16, fontSize:24, background:'none', border:'none', cursor:'pointer', color: isDark ? '#fff' : '#222'}}>&times;</button>
          <h1 style={{color:isDark?'#fff':'#333', fontSize:28, marginBottom:16}}>📄 PPS-2 Resources</h1>
          <div style={{marginBottom:20, display:'flex', flexDirection:'row', flexWrap:'wrap', justifyContent:'center', alignItems:'center', gap:16}}>
            {error && <p style={{ color: 'red', width: '100%' }}>{error}</p>}
            {files.map(url => {
              const filename = url.split('/').pop();
...
function Maths2FilesPopup({ open, onClose, isDark }) {
  const [files, setFiles] = useState([]);
  const [error, setError] = useState('');
  const [pdfModalOpen, setPdfModalOpen] = useState(false);
  const [pdfToView, setPdfToView] = useState('');
  ...
  useEffect(() => {
    if (!open) return;
    fetch('http://localhost:5000/list-maths2-files')
      .then(res => res.json())
      .then(data => {
        setFiles(data);
        setError('');
  ...
  return (
    <>
      <div style={{position: 'fixed', top:0, left:0, width:'100vw', height:'100vh', background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center'}}>
        <div style={{background:isDark?'#232a39':'#fff', borderRadius:12, padding:24, width:'90vw', maxWidth:800, maxHeight:'90vh', overflow:'auto', position:'relative'}}>
          <button onClick={onClose} style={{position:'absolute', top:12, right:16, fontSize:24, background:'none', border:'none', cursor:'pointer', color: isDark ? '#fff' : '#222'}}>&times;</button>
          <h1 style={{color:isDark?'#fff':'#333', fontSize:28, marginBottom:16}}>📄 Maths-2 Resources</h1>
          <div style={{marginBottom:20, display:'flex', flexDirection:'row', flexWrap:'wrap', justifyContent:'center', alignItems:'center', gap:16}}>
            {error && <p style={{ color: 'red', width: '100%' }}>{error}</p>}
            {files.map(url => {
              const filename = url.split('/').pop();
...
function ChemistryFilesPopup({ open, onClose, isDark }) {
  const [files, setFiles] = useState([]);
  const [error, setError] = useState('');
  const [pdfModalOpen, setPdfModalOpen] = useState(false);
  const [pdfToView, setPdfToView] = useState('');
  ...
  useEffect(() => {
    if (!open) return;
    fetch('http://localhost:5000/list-chemistry-files')
      .then(res => res.json())
      .then(data => {
        setFiles(data);
        setError('');
  ...
  return (
    <>
      <div style={{position: 'fixed', top:0, left:0, width:'100vw', height:'100vh', background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center'}}>
        <div style={{background:isDark?'#232a39':'#fff', borderRadius:12, padding:24, width:'90vw', maxWidth:800, maxHeight:'90vh', overflow:'auto', position:'relative'}}>
          <button onClick={onClose} style={{position:'absolute', top:12, right:16, fontSize:24, background:'none', border:'none', cursor:'pointer', color: isDark ? '#fff' : '#222'}}>&times;</button>
          <h1 style={{color:isDark?'#fff':'#333', fontSize:28, marginBottom:16}}>📄 Chemistry Resources</h1>
          <div style={{marginBottom:20, display:'flex', flexDirection:'row', flexWrap:'wrap', justifyContent:'center', alignItems:'center', gap:16}}>
            {error && <p style={{ color: 'red', width: '100%' }}>{error}</p>}
            {files.map(url => {
              const filename = url.split('/').pop();
...
export default function OneSemII() {
  const navigate = useNavigate();
  const [showEnglishPopup, setShowEnglishPopup] = useState(false);
  const [showPPS2Popup, setShowPPS2Popup] = useState(false);
  const [showMaths2Popup, setShowMaths2Popup] = useState(false);
  const [showChemistryPopup, setShowChemistryPopup] = useState(false);
    ...
    <div className={`min-h-screen w-full ${isDark ? 'bg-[#181e29]' : 'bg-gray-100'}`} style={{position:'relative',overflow:'hidden'}}>
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className={`${isDark ? 'bg-[#232a39] border-[#232a39]' : 'bg-white border-gray-200'} border p-8 rounded-2xl w-full max-w-2xl shadow-xl relative`}>
          ...
          <h2 className={`${isDark ? 'text-white' : 'text-black'} text-2xl font-bold mb-8`}>1st Year - Semester II Subjects</h2>
          <div className="flex flex-col gap-4">
            <button className="py-3 bg-red-300 text-black rounded-lg font-semibold hover:bg-red-400 transition duration-300" onClick={() => setShowMaths2Popup(true)}>Maths-2</button>
            <button className="py-3 bg-red-300 text-black rounded-lg font-semibold hover:bg-red-400 transition duration-300" onClick={() => setShowEnglishPopup(true)}>English</button>
            <button className="py-3 bg-red-300 text-black rounded-lg font-semibold hover:bg-red-400 transition duration-300" onClick={() => setShowChemistryPopup(true)}>Chemistry</button>
            <button className="py-3 bg-red-300 text-black rounded-lg font-semibold hover:bg-red-400 transition duration-300" onClick={() => setShowPPS2Popup(true)}>PPS-2</button>
          </div>
        </div>
      </div>
      <Maths2FilesPopup open={showMaths2Popup} onClose={() => setShowMaths2Popup(false)} isDark={isDark} />
      <EnglishPdfPopup open={showEnglishPopup} onClose={() => setShowEnglishPopup(false)} isDark={isDark} />
      <ChemistryFilesPopup open={showChemistryPopup} onClose={() => setShowChemistryPopup(false)} isDark={isDark} />
      <PPS2FilesPopup open={showPPS2Popup} onClose={() => setShowPPS2Popup(false)} isDark={isDark} />
      {showSettings && (
